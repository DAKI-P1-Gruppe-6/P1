...
filtered_data = DD[~DD["diabetes_stage"].isin(["Type 1", "Gestational"])].copy()
...
#                                Linear Scores
lin_svc_recall_score = recall_score(y_test, lin_y_pred, average="weighted")
lin_svc_accuracy_score = accuracy_score(y_test, lin_y_pred,)
lin_svc_precision_score = precision_score(y_test, lin_y_pred, average="weighted")
...
rbf_svc = SVC(kernel="rbf", random_state=42, max_iter=1000)
...
rbf_svc_recall_score = recall_score(y_test, rbf_pred, average="weighted")
rbf_svc_accuracy_score = accuracy_score(y_test, rbf_pred)
rbf_svc_precision_score = precision_score(y_test, rbf_pred, average="weighted")
